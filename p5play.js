...
p5.prototype.registerMethod("init", function p5playInit() {
	...
	const fixRound = (val, slop) =>
		Math.abs(val - Math.round(val)) <= (slop || pl.Settings.linearSlop)
			? Math.round(val)
			: val;
	...
	this.Sprite = class {
		...
		constructor(x, y, w, h, collider) {
			...
			Object.defineProperty(this._pos, "x", {
				get() {
					...
					let x =
						(_this.body.getPosition().x / _this.tileSize) * $.world.meterSize;
			...
			Object.defineProperty(this._pos, "y", {
				get() {
					...
					let y =
						(_this.body.getPosition().y / _this.tileSize) * $.world.meterSize;
		...
		get rotation() {
			...
			return $.p5play.friendlyRounding
				? fixRound(val, pl.Settings.angularSlop)
				: val;
		}
		set rotation(val) {
			if (this.body) {
		...
		_display() {
			...
			if (!this._userDefinedDraw) {
				...
				if (!this._totalWidth) {
					largestSide =
						this._h !== undefined ? Math.max(this._w, this._h) : this._w;
				} else {
	...
	this.World = class extends planck.World {
		...
		getSpritesAt(x, y, group, cameraActiveWhenDrawn) {
			...
			const convertedPoint = new pl.Vec2(
				x / $.world.meterSize,
				y / $.world.meterSize
			);
				...
				for (let l of ["A", "B"]) {
					...
					for (let axis of ["x", "y"]) {
						Object.defineProperty(this[prop], axis, {
							...
							set(val) {
								_this._j["m_localAnchor" + l][axis] =
									(val / $.world.meterSize) * _this["sprite" + l].tileSize;
	...
	this.SliderJoint = class extends $.Joint {
		...
		get lowerLimit() {
			return (
				(this._j.getLowerLimit() / this.spriteA.tileSize) * $.world.meterSize
			);
		}
		set lowerLimit(val) {
			if (!this._j.isLimitEnabled()) {
		...
		get upperLimit() {
			return (
				(this._j.getUpperLimit() / this.spriteA.tileSize) * $.world.meterSize
			);
		}
		set upperLimit(val) {
			if (!this._j.isLimitEnabled()) {
...
`;
	...
	this.renderStats = (x, y) => {
		console.error(
			"p5play: renderStats() function is deprecated. Use `p5play.renderStats = true` instead."
		);
	};
});
